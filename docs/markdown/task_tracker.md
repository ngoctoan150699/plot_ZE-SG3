...
- [ ] Thêm logic đọc/ghi cấu hình các profile servo vào `python/infrastructure/app_settings.py`
- [ ] Thêm logic đọc/ghi cài đặt vùng dữ liệu Operating (`operating_setups`) vào `python/infrastructure/app_settings.py`
- [ ] Thêm logic đọc/ghi đường dẫn thư mục lưu báo cáo (`report_paths`) vào `python/infrastructure/app_settings.py`

### [ ] Session 4: Viết Bộ Tiện Ích Chuẩn Hóa Chữ Tester (Tester String Helper)
- [ ] Viết hàm loại bỏ dấu tiếng Việt và tự động viết hoa (`remove_vietnamese_diacritics`)
- [ ] Tạo unit test chạy độc lập kiểm tra kết quả chuẩn hóa chuỗi Tester

### [ ] Session 5: Tạo Module Dịch Song Ngữ (Bilingual Dictionary & Service)
- [ ] Tạo tệp `python/ui/i18n.py` chứa từ điển tiếng Anh và tiếng Việt
- [ ] Lập trình class `I18n` hỗ trợ phương thức dịch nhanh `t(key)` và đổi ngôn ngữ `toggle()`

### [ ] Session 6: Thiết Kế Bộ Điều Khiển PLC Modbus (PLC Driver)
- [ ] Khai báo interface `IPLCServoController` trong `python/application/interfaces.py`
- [ ] Lập trình driver `PLCServoController` trong `python/infrastructure/plc_servo_controller.py` thực hiện ghi thanh ghi Modbus
- [ ] Hỗ trợ chế độ chạy mô phỏng `DummyPLCServoController` khi không kết nối thiết bị thực

### [ ] Session 7: Lập Trình Chu Trình Đo Động Cơ (Servo Sequence Service)
- [ ] Lập trình chạy chế độ Operating (3 cycle lặp lại 0 -> + -> - -> 0) trong `python/application/servo_service.py`
- [ ] Lập trình chạy chế độ Breakaway (0 -> + -> 0, chỉ ghi chiều đi) trong `python/application/servo_service.py`
- [ ] Tích hợp tính toán góc liên tục theo công thức: `angle = speed * time * 6.0`

### [ ] Session 8: Xây Dựng Bộ Tính Toán Kết Quả Đo (Measurement Calculator)
- [ ] Lập trình thuật toán cắt lấy 80% dữ liệu ở giữa của cycle 3 trong `python/application/measurement_service.py`
- [ ] Lập trình tính toán Avg, Max, Min cho Operating và lấy Max cho Breakaway
- [ ] Lập trình bộ đánh giá OK/NG dựa trên so sánh giới hạn tiêu chuẩn (Spec Limits)
