...
### [x] Session 9: Viết Dịch Vụ Lưu Trữ Báo Cáo Tự Tăng (Report & File Service)
- [x] Viết logic sinh tên tệp tự động tăng số thứ tự mẫu `yymmdd-T-PartNo-P-Team-##.csv`
- [x] Lập trình lưu đồng thời tệp raw CSV và tệp CTR report tương ứng vào đúng thư mục cấu hình

### [x] Session 10: Tạo Đồ Thị Góc Thời Gian Thực (Torque-Angle Live Chart)
- [x] Tạo widget `TorqueAnglePlot` thừa kế `FigureCanvas` trong `python/ui/widgets/torque_angle_plot.py`
- [x] Tối ưu hóa việc vẽ đồ thị góc xoay thời gian thực bằng Matplotlib blit

### [x] Session 11: Cập Nhật Vùng Đồ Thị & Bố Cục UI Chính (Main UI Splitter & Language)
- [x] Xóa bỏ tab Converter và nút "Import to Converter" trên MainWindow
- [x] Chia đôi màn hình đồ thị chính bằng `QSplitter` hiển thị song song Đồ thị Time và Angle
- [x] Tích hợp nút chọn ngôn ngữ Anh-Việt, dịch động các nhãn và nút bấm giao diện chính
- [x] Đổi tên nhãn "Factor K" thành "Calibration"

### [x] Session 12: Thiết Kế Lại Nhóm Thông Tin Tab Plot Viewer (Plot Viewer Layout)
- [x] Redesign nhóm "Report Info" QGroupBox trong `draw_plot.py` với các input mới (Part No viết hoa, Tester viết hoa không dấu...)
- [x] Bổ sung các ô chọn thư mục lưu **CSV File path** và **Report File path** kèm nút `[📂]`
- [x] Tích hợp nút **Save the Report** gọi dịch vụ lưu kép báo cáo
- [x] Đồng bộ hóa nút **Import to Plot Viewer** chuyển đổi dữ liệu và tự động nhảy tab màn hình


### [x] Session 13: Tích Hợp Entry Point & Chạy Thử (System Integration)
- [x] Đăng ký khởi tạo các service mới và inject trong `python/main.py`
- [x] Chạy thử nghiệm toàn hệ thống bằng Python 3.14 không xảy ra lỗi nạp thư viện
